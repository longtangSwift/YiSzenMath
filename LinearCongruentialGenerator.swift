...
class LinearCongruentialGenerator: RandomNumberGenerator {
    ...
    private let a = 3877.02 + abs((NSDate(timeIntervalSince1970: 14234).timeIntervalSinceReferenceDate - NSDate(timeIntervalSinceNow: 1).timeIntervalSinceReferenceDate - 2 * NSDate(timeIntervalSince1970: 14234).timeIntervalSinceReferenceDate) )
    ...
    func random(range: Int)->Double
    {
        let x = random()
        return Double(Int(Double(range) * x))
    }
}
